...
contract DefiLottery is Owned {
    ...
    uint public upDownLotteryStartTime;
    ...
    //random lottery start time of day
    uint public randomLotteryStartTime;
    ...
    { 
       ...
       uint startTime = upDownLotteryStartTime.add(upDownLotteryTimeCycle.mul(calUpDownRound));
       //each round end time
       ...
       if (updownGameMap[calUpDownRound].upAmount==0 && updownGameMap[calUpDownRound].downAmount==0) {
            ...
            updownGameMap[calUpDownRound] = UpDownGameItem(0,0,0,0,0,upStakers,downStakers);
       }
       ...
       if (randomGameMap[calRandomRound].curRandomStakeIdx==0) {
           ...
           randomGameMap[calRandomRound] = RandomGameItem(0,0,0,stakingTime,accStakeRange,extraPrize);
       }
        ...
        public
    {
       ...
       uint endTime = upDownLotteryStartTime.add(upDownLotteryTimeCycle.mul(curUpDownRound + 1));
       require(now>endTime);
        ...
        updownGameMap[curUpDownRound].feeTotal =  total.mul(feeRatio).div(DIVISOR);
        ...
        public
    {
       ...
       //end time for this round
       uint endTime = randomLotteryStartTime.add(randomLotteryTimeCycle.mul(curRandomRound + 1));
       require(now>endTime);
        ...
        for(i= randomGameMap[curRandomRound].startUpdownRound;i<curRandomRound;i++) {
            totalPrize = totalPrize.add(updownGameMap[i].feeTotal);
        }
        ...
        public
    {
        ...
        upDownLotteryStartTime = _startTime.mod(_updownLtryTimeCycle);
        upDownLotteryTimeCycle = _updownLtryTimeCycle;
        upDownLtrstopTimeSpanInAdvance = _stopTimeSpanInAdvance;
    }
    
    function setRandomLotteryTime(uint _startTime,uint _randomLotteryTimeCycle) 
        onlyOwner
        notHalted
        public
    {
       //only set one time
       require(randomLotteryStartRN == 0 );   
       randomLotteryStartRN = _startTime.div(_randomLotteryTimeCycle); 
       
       randomLotteryStartTime = _startTime.mod(_randomLotteryTimeCycle);
